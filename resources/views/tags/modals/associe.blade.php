        ...
        <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
          <div class="sm:flex sm:items-start">
            ...
            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
              <h3 id="dialog-title" class="text-base font-semibold text-gray-900">Attribuer les Tags RFID </h3>
                <div class="mt-2 mb-3">
                  <label class="form-label">{{ __('tag.owner_id') }}</label>
                    <select id="owner_id" class="form-control border border-2 p-2 {{ $errors->has('owner_id') ? ' is-invalid' : '' }}" required>
                      ...
                      @foreach($owners as $nin=>$name)
                      <option value="{{ $nin }}" {{ old('owner_id') == $nin ? 'selected' : '' }}>{{ $name }}</option>
                      @endforeach
                    </select>
                </div>
              </div>
          ...
          <div class="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
            ...
            <button type="button" command="close" class="mt-3 nline-flex justify-center rounded-md bg-gradient-primary px-3 py-2 text-sm font-semibold text-white" onclick="fct();">{{ __('Save')}}</button> 
            <button type="button" command="close" commandfor="dialog" class="mt-3 inline-flex w-full justify-center rounded-md bg-warning px-3 py-2 text-sm font-semibold text-gray-900 shadow-xs inset-ring inset-ring-gray-300 hover:bg-gray-50 sm:mt-0 sm:w-auto">Cancel</button>
          </div>          
      </el-dialog-panel>
    </div>
  </dialog>
